...
**Tech Stack:** Next.js 16 (App Router, Turbopack default), React 19.2, TypeScript strict, Tailwind v4, Supabase (Postgres + Auth + SSR — remote project, no local Docker), Vitest, Playwright, npm.
...
**Workspace:** `feat/tracker-app` branch off `master`.

## Plan revisions (2026-05-17, post-version-check)

Applied to the steps below; called out here so the deltas are visible:

- **Next 15 → 16.** Turbopack is the default (no flag needed); `next lint` was removed (use `eslint .` directly); root `middleware.ts` is renamed to **`proxy.ts`** with an exported `proxy()` function. The async-cookies/searchParams APIs in the plan are unchanged — they were already async-only in our draft.
- **No local Supabase.** Task 4 uses a real Supabase project at supabase.com. The migration SQL is applied via the SQL editor in the dashboard. No `supabase start`, no Docker, no CLI required.
- **E2E** runs against the real project (Tasks 18–19). Test users (`e2e@example.com`, `toggle-e2e@example.com`, `import-e2e@example.com`) accumulate in production Auth — clean them up manually from the dashboard when needed.
...
```bash
npx create-next-app@16 . --typescript --tailwind --app --src-dir=false --import-alias="@/*" --no-eslint --use-npm --yes
```
...
```bash
npm install -D tailwindcss@^4 @tailwindcss/postcss@^4 prettier prettier-plugin-tailwindcss eslint@^9 eslint-config-next@^16 @types/node@^22
...
## Task 4: Supabase remote project + `owned_pokemon` migration

**Files:**
- Create: `supabase/migrations/20260517120000_owned_pokemon.sql` (kept as canonical SQL even though we apply it manually)
- Modify: `.env.example`

**Prerequisite:** A free Supabase account at supabase.com. No Docker, no CLI required.
...
2. Click "New project"
3. Name it `national-pokedex` (or anything)
4. Region: closest to you
5. Save the database password somewhere safe
6. Wait ~2 minutes for provisioning

When ready, from the project dashboard:
- **Project Settings → API → Project URL** — copy as `NEXT_PUBLIC_SUPABASE_URL`
- **Project Settings → API → Project API keys → anon (public)** — copy as `NEXT_PUBLIC_SUPABASE_ANON_KEY`
- **Project Settings → API → Project API keys → service_role (secret)** — copy as `SUPABASE_SERVICE_ROLE_KEY`
...
This file is kept as the canonical schema record. We apply it via the dashboard SQL editor in the next step.

- [ ] **Step 3: Apply the migration to the remote project (user action)**

In the Supabase dashboard for your project:

1. Open **SQL Editor**
2. Click "New query"
3. Paste the entire contents of `supabase/migrations/20260517120000_owned_pokemon.sql`
4. Click **Run**

Expected: "Success. No rows returned."

- [ ] **Step 4: Verify the table + policy in the dashboard**

In the dashboard:

1. **Table Editor → owned_pokemon** — verify columns `user_id`, `dex_number`, `acquired_at`, with the composite PK
2. **Authentication → Policies → owned_pokemon** — verify `owner_full_access` exists with `FOR ALL` and the `auth.uid() = user_id` clauses
...
(Add the Vercel production URL later, in Task 20.)

- [ ] **Step 6: Write `.env.example`**

```
# Supabase remote project — get values from supabase.com/dashboard → Project Settings → API
NEXT_PUBLIC_SUPABASE_URL=
NEXT_PUBLIC_SUPABASE_ANON_KEY=
SUPABASE_SERVICE_ROLE_KEY=
```

- [ ] **Step 7: Create `.env.local` (NOT committed) with the real values**

```bash
cp .env.example .env.local
```

Open `.env.local` and paste the three values from Step 1.

- [ ] **Step 8: Commit migration SQL + .env.example**

```bash
git add supabase/migrations .env.example
git commit -m "Add owned_pokemon migration and env template"
```
...
- [ ] **Step 3: Proxy helper (Next 16 renamed middleware → proxy)**
...
- [ ] **Step 4: Wire root proxy (Next 16 file convention)**

Create `proxy.ts` at the project root (NOT `middleware.ts` — that name is deprecated in Next 16):
...
git commit -m "Add Supabase server/browser/proxy clients"
```
...
The dev server should **not** be running locally (Playwright will start it via `webServer`). `.env.local` must contain `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY` pointing at the real Supabase project.
...
**Caveat:** test users (`e2e@example.com`, etc.) get created in your production Auth table. Delete them from Supabase Dashboard → Authentication → Users when you want to clean up.
...
2. Apply `supabase/migrations/20260517120000_owned_pokemon.sql` via the dashboard SQL editor.
...
4. Copy `.env.example` → `.env.local` and paste your project URL + anon key + service role key.
...
Visit http://localhost:3000, enter your email, and click the magic-link email from
Supabase.
...
npm run test:e2e              # E2E (Playwright) — hits your real Supabase project
```

E2E creates test users in your Supabase Auth table. Clean them up periodically via
the dashboard (Authentication → Users) — search for `*-e2e@example.com`.
...
2. In Supabase dashboard → **Authentication → URL Configuration**, add the Vercel
   production URL as a Site URL (or Redirect URL) so magic-link emails point there.
3. Push to `master` (or your default branch) → Vercel builds → live.

If you want a separate prod Supabase project, create it the same way as the dev one
and re-apply the migration via the dashboard SQL editor.
