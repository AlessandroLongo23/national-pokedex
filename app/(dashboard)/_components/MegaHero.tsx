...
import { CARD_INDEX_BY_MEGA, POKEDEX, SPECIES } from "@/lib/data";
import type { Generation, MegaForm } from "@/lib/data/types";
...
export function MegaHero({ form }: { form: MegaForm }) {
      ...
      <div className="grid gap-5 py-5 md:grid-cols-[176px_1fr] md:items-start md:gap-7 md:py-6">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            <span className="inline-flex h-[18px] min-w-[18px] items-center justify-center rounded-sm bg-mega/85 px-1 text-[10px] font-bold leading-none text-bg">
              {form.isPrimal ? "P" : "M"}
            </span>
            <span className="eyebrow text-mega">
              {kind} Evolution · {region}
            </span>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-start md:gap-8">
            ...
            <div className="flex md:justify-end">
              <MegaBinderSlot formKey={form.formKey} />
            </div>
          </div>
...
function MegaBinderSlot({ formKey }: { formKey: string }) {
  const { ownedCountForMegaForm } = useOwnedCards();
  const total = CARD_INDEX_BY_MEGA[formKey]?.length ?? 0;
  const owned = ownedCountForMegaForm(formKey);
    ...
    <div className="flex flex-col items-start gap-2 md:items-end">
      <div
        className={[
          ...
          filled
            ? "border-mega/60 bg-mega/15 text-mega"
            : "border-border-strong/70 bg-panel-2 text-muted",
        ].join(" ")}
      >
        <span
          aria-hidden
          className={["h-1.5 w-1.5 rounded-full", filled ? "bg-mega" : "border border-muted/70"].join(" ")}
        />
        <span>Mega form {filled ? "owned" : "unowned"}</span>
      </div>
      <div className="flex items-baseline gap-1.5 nums">
        <span
          className={[
            "text-4xl font-semibold leading-none tracking-tight md:text-5xl",
            filled ? "text-mega" : "text-text",
          ].join(" ")}
        >
          {owned}
        </span>
