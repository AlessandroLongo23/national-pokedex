...
/** Regional-variant counterpart of MegaVariantPicker: lists the card variants
 * that print a given regional form so they can be marked owned/wishlisted. */
export function VariantCardPicker({ form, onClose }: Props) {
  const [cards, setCards] = useState<CardEntry[] | null>(null);
  const [loading, setLoading] = useState(false);

  const variantKey = form?.variantKey ?? null;

  useEffect(() => {
    if (variantKey == null) return;
    setLoading(true);
    setCards(null);
    fetch(`/api/cards-by-variant/${variantKey}`)
  ...
  const count = cards?.length ?? CARD_INDEX_BY_VARIANT[form.variantKey]?.length ?? 0;
        ...
        <header className="flex items-center gap-3 border-b border-border p-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-muted nums">
              <span className="rounded-sm bg-variant/85 px-[3px] text-[9px] font-bold leading-none text-bg">
                {REGION_BADGE[form.region]}
              </span>
              <span>
                {form.region.charAt(0).toUpperCase() + form.region.slice(1)} · {cards ? count : "…"} card
                {count === 1 ? "" : "s"}
              </span>
            </div>
          ...
          <Link
            ...
            onClick={onClose}
            className="inline-flex items-center gap-1 rounded-md border border-border bg-panel-2 px-3 py-1.5 text-xs text-muted transition hover:border-variant hover:text-variant"
          >
            Full details
        ...
        <div className="max-h-[calc(88vh-92px)] overflow-y-auto p-3">
          ...
          {cards && cards.length === 0 && (
            <div className="py-12 text-center text-sm text-muted">
              No card variants found for this regional form.
            </div>
          )}
          {cards && cards.length > 0 && (
